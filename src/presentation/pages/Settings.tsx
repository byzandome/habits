...
import { appUsageUseCases, settingsUseCases } from '@/infrastructure/container';
import { clearIconMemoryCache } from '@/presentation/hooks/useAppIcon';
...
export function Settings() {
  ...
  const handleClearCache = async () => {
    setClearing(true);
    try {
      await appUsageUseCases.clearIconCache();
  ...
  const handleDeleteData = async () => {
    ...
    setDeleting(true);
    try {
      await settingsUseCases.clearAllData();
      clearIconMemoryCache();
      setDeleted(true);
  ...
  useEffect(() => {
    settingsUseCases.getSettings().then((s) => {
      setSettings(s);
      setLoading(false);
    });
  }, []);
  ...
  const handleSave = async () => {
    await settingsUseCases.setSettings(settings);
    setSaved(true);
