import type { Interval, Session } from '../types';
...
export function Sessions() {
  ...
  useEffect(() => {
    setLoading(true);
    api.getSessionForDate(date).then((s) => {
      setSession(s);
      if (s?.id) {
        api.getIntervalsForSession(s.id).then(setIntervals);
      } else {
        setIntervals([]);
      }
      setLoading(false);
          ...
          <div className="card session-row">
            ...
            <div className="session-content">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4 }}>
                <span className="type-badge" style={{ background: 'rgba(34,197,94,0.12)', color: '#22C55E' }}>
                  Active: {formatDuration(session.active_secs)}
                </span>
                {session.idle_secs > 0 && (
                  <span className="type-badge" style={{ background: 'rgba(100,116,139,0.12)', color: '#64748B' }}>
                    Idle: {formatDuration(session.idle_secs)}
                  </span>
                )}
                {session.locked_secs > 0 && (
                  <span className="type-badge" style={{ background: 'rgba(245,158,11,0.12)', color: '#F59E0B' }}>
              ...
              {intervals.map((iv) => (
                <div key={iv.id} className="card session-row" style={{ marginBottom: 6 }}>
                  <div
                    ...
                    style={{
                      background:
                        iv.type === 'active' ? '#22C55E' :
                        iv.type === 'idle'   ? '#64748B' :
                        iv.type === 'locked' ? '#F59E0B' : '#94A3B8',
                    }}
                  />
                  <div className="session-content">
                    <div className="session-time-range">
                      ...
                      <span className="type-badge" style={{ marginLeft: 10 }}>{iv.type}</span>
                    </div>
